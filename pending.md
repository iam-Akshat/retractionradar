...
- [ ] Replace the placeholder metadata base URL with the final public deployment URL.
- [ ] Configure `OPENALEX_API_KEY` as a hosting secret; confirm it never appears in client bundles or repository history.
...
- [ ] Validate all seven WebMCP tools in ChatGPT’s built-in browser.
- [ ] Test `.bib` upload with at least one real user-owned bibliography.
- [ ] Confirm agent-staged repairs cannot be approved through WebMCP.
- [ ] Confirm rejected and pending candidates never alter exported BibTeX.
- [ ] Run production build, focused accessibility review, responsive review, and dependency audit.
